...
export function TicketTemplateManager() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json.error || "Не удалось загрузить шаблоны");
  ...
  async function onSubmitNew(e: React.FormEvent) {
    ...
    if (salePriceVnd <= 0) { setFormError("Введите цену продажи"); return; }
    setSaving(true);
    try {
      ...
      const json = await res.json();
      if (!res.ok) throw new Error(typeof json.error === "string" ? json.error : "Ошибка");
      setForm(EMPTY_FORM);
      setShowForm(false);
      await load();
    } catch (e) {
  ...
  return (
    <section className="card mb-3">
      <div className="flex items-center justify-between gap-2">
        <h2 className="text-base font-semibold">Шаблоны билетов</h2>
        <button
          type="button"
      ...
      {showForm && (
        <form onSubmit={onSubmitNew} className="mt-3 space-y-3 rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] p-3">
          ...
          <label className="flex flex-col gap-1 text-sm">
            ...
            <input
              type="text"
              placeholder="VinWonders — пенсионный"
              value={form.name}
          ...
          <label className="flex flex-col gap-1 text-sm">
            <span className="text-[var(--muted)]">Цена продажи (₫)</span>
            <input
              type="text"
              inputMode="numeric"
              placeholder="950.000"
          ...
          <div className="grid grid-cols-2 gap-2">
            <label className="flex flex-col gap-1 text-sm">
              <span className="text-[var(--muted)]">Комиссия офиса</span>
            ...
            <label className="flex flex-col gap-1 text-sm">
              <span className="text-[var(--muted)]">Комиссия менеджера</span>
              <div className="flex gap-1">
                <select
      ...
      {loading && <p className="mt-2 text-sm text-[var(--muted)]">Загрузка…</p>}
      ...
      {!loading && !error && (
        <div className="mt-3 space-y-4">
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-[var(--muted)]">
                        {formatVndPlain(tpl.sale_price_vnd)} · офис{" "}
                    ...
                    >
                      {toggling === tpl.id ? "…" : tpl.active ? "Отключить" : "Включить"}
                    </button>
                  </li>
                ))}
              </ul>
          ...
          {templates.length === 0 && (
            <p className="text-sm text-[var(--muted)]">Нет шаблонов. Создайте первый.</p>
          )}
        </div>
      )}
    </section>

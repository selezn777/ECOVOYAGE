  ...
  return (
    <main className="app-wrap app-wrap--wide">
      ...
      {!report ? (
        ...
        <>
          {!isAllTimeRange && tourSnap.incomeVnd === 0 && totalHandoverVnd === 0 && report.paymentsIncomeVnd === 0 ? (
            <section className="card mb-3 border-amber-200/80 bg-amber-50/50 dark:border-amber-900/50 dark:bg-amber-950/20">
              <p className="text-sm text-amber-900 dark:text-amber-200">{t("emptyPeriodNotice")}</p>
              <Link
                href={`/accounting/reports?from=${encodeURIComponent(REPORT_ALL_TIME_FROM)}&to=${encodeURIComponent(today)}`}
                className="mt-2 inline-flex min-h-[40px] items-center justify-center rounded-xl bg-amber-700 px-4 py-2 text-[13px] font-semibold text-white hover:bg-amber-800 dark:bg-amber-600 dark:hover:bg-amber-500"
              >
                {t("emptyPeriodShowAllTime")}
              </Link>
            </section>
          ) : null}
          <AccountingReportPeriodSummary

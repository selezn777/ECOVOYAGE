...
import { getManagerCashHandoverAllToursSummary } from "@/lib/data";
...
export async function GET(_request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { id: managerId } = await ctx.params;
  const data = await getManagerCashHandoverAllToursSummary(managerId);

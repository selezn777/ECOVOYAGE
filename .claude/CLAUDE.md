...
## ⚠️ ОБЯЗАТЕЛЬНО — 3 ЯЗЫКА ВСЕГДА

**Любая работа с текстом UI (новые экраны, формы, кнопки, сообщения об ошибках,
редизайн существующих страниц) — сразу на RU/EN/VI через next-intl
(`messages/{ru,en,vi}.json`).** Хардкод русских строк недопустим — даже временно.
Это касается и серверных, и клиентских компонентов, и API-ошибок, отображаемых
пользователю.
